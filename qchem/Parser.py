...
class OrcaOutput:
    def __init__(self, file_path: str):
        ...
        print(f"Final Timings: {self.final_timings}")
        self.mayer_population = self.get_mayer_population()
        print(f"Mayer Population: {self.mayer_population}")
        self.loedwin = self.get_loewdin_charges()
        print(f"Loewdin Charges: {self.loedwin}")
        self.dipole = self.get_dipole_vector()
        print(f"Dipole: {self.dipole}")
        self.absolutedipole = self.get_dipole_magnitude()
        print(f"Absolute Dipole: {self.absolutedipole}")
        ...
        self.IR_frequencies = self.get_ir_frequencies() if "FREQ" in self.calc_types else None
        self.chemical_shifts = self.get_chemical_shifts() if "NMR" in self.calc_types else None
    ...
    def save_to_txt(self, output_path: str):
        ...
        with open(output_path, "w") as file:
            ...
            if "GOAT" in self.calc_types:
                ...
                if self.goat_summary:
                    file.write("Summary:\n")
                    file.write(f"Conformers below 3 kcal/mol: {self.goat_summary['conformers_below_3kcal']}\n")
                    file.write(f"Lowest energy conformer: {self.goat_summary['lowest_energy']} Eh\n")
                    file.write(f"Sconf at 298.15 K: {self.goat_summary['sconf']} cal/(molK)\n")
                    file.write(f"Gconf at 298.15 K: {self.goat_summary['gconf']} kcal/mol\n\n")

    def read_xyz_file(self, path: str) -> pd.DataFrame:
    ...
    def determine_calculation_type(self) -> list[str]:
        ...
        print(f"Calculation Types: {self.calc_types}")
